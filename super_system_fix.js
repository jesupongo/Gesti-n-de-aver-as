...
console.log("Applying SUPER SYSTEM FIX...");

// 1. Foundation: App State & Roles
...
content = content.replace('iniciarSesionComo:(e,t)=>{r(e),a(t),o(e===`personal`?`panel-personal`:`panel-admin`)}})', 'iniciarSesionComo:(rol_f,nom,id_v)=>{localStorage.setItem(`v`,rol_f===`personal`?`panel-personal`:`panel-admin`),localStorage.setItem(`r`,rol_f),localStorage.setItem(`n`,nom),localStorage.setItem(`u`,id_v),r(rol_f),a(nom),s(id_v),t(rol_f===`personal`?`panel-personal`:`panel-admin`)}})');
...
content = content.replace('e===`crear-usuario`&&(0,x.jsx)(ce,{navegar:o})', 'e===`crear-usuario`&&(0,x.jsx)(ce,{navegar:t})');
content = content.replace('e===`usuarios-averias`&&(0,x.jsx)(re,{navegar:o,rolUsuario:n})', 'e===`usuarios-averias`&&(0,x.jsx)(re,{navegar:t,rol_u:n})');
...
content = content.replace('function re({navegar:e,rolUsuario:t})', 'function re({navegar:e,rol_u:rol_u})');
...
// VistaAdmin Header Buttons
const header_box_pattern = /className:`botones-header`,style:\{display:`flex`,gap:`0\.75rem`,flexShrink:0\},children:t==="admin"&&\(0,x\.jsxs\)\(x\.Fragment,\{children:\[\(0,x\.jsx\)\(`button`,\{onClick:_,className:`boton boton-secundario boton-header`,style:\{marginTop:0,whiteSpace:`nowrap`\},children:`Usuarios y Averías`\}\),\(0,x\.jsx\)\(`button`,\{onClick:v,className:`boton boton-secundario boton-header`,style:\{marginTop:0,whiteSpace:`nowrap`\},children:`\+ Crear Usuario`\}\)\]\}\)/;
const header_box_replacement = `className:"botones-header",style:{display:"flex",gap:"0.75rem",flexShrink:0},children:(0,x.jsxs)(x.Fragment,{children:[
  ...
  (rol_u=="admin"||rol_u=="ADMINISTRADOR") && (0,x.jsx)("button",{onClick:_,className:"boton boton-secundario boton-header",style:{marginTop:0,whiteSpace:"nowrap"},children:"Usuarios"}),
...
content = content.replace(header_box_pattern, header_box_replacement);
...
// 5. VistaUsuariosAverias (re) REAL DATA
content = content.replace('function re({navegar:e,rol_u:rol_u}){let[t,n]=(0,b.useState)(``),[r,i]=(0,b.useState)([])', 'function re({navegar:e,rol_u:rol_u}){let[t,n]=(0,b.useState)(``),[r,i]=(0,b.useState)([]);(0,b.useEffect)(()=>{fetch(`/user`).then(res=>res.json()).then(data=>{i(data.map(u=>({id:u.id,nombre:u.nombre,rol:u.rol===`ADMINISTRADOR`?`Administrador`:u.rol===`MANTENIMIENTO`?`T\u00e9cnico`:`Personal`,correo:u.email,averiasComunicadas:[]})))})},[])');
// Fix roles in re
content = content.replace('rolUsuario!==`admin`','!(rol_u==`admin`||rol_u==`ADMINISTRADOR`)');

// 6. VistaPersonal (ae) REAL FETCH
...
console.log("SUPER SYSTEM FIX COMPLETED.");
