...
// 2. Component Routing in App (Using specific ie component name)
...
content = content.replace('e===`usuarios-averias`&&(0,x.jsx)(ie,{navegar:o,rolUsuario:n})', 'e===`usuarios-averias`&&(0,x.jsx)(ie,{navegar:t,rol_u:n})');
content = content.replace('e===`crear-usuario`&&(0,x.jsx)(re,{navegar:o})', 'e===`crear-usuario`&&(0,x.jsx)(re,{navegar:t})');
content = content.replace('e===`comunicar-averia`&&(0,x.jsx)(C,{navegar:o})', 'e===`comunicar-averia`&&(0,x.jsx)(C,{navegar:t,id_u:u})');

// 3. Signature Patching
...
// 4. VistaAdmin (ne) Logic with state segmentation
...
// Admin Header Buttons (Literal from bundle snippet)
...
// 5. VistaUsuariosAverias (ie) Surgical Fix
const ie_anchor = 'function ie({navegar:e,rol_u:rol_u}){let[n,r]=(0,b.useState)(``),[i,a]=(0,b.useState)(w)';
const ie_replacement = 'function ie({navegar:e,rol_u:rol_u}){let[n,r]=(0,b.useState)(``),[i,a]=(0,b.useState)([]);(0,b.useEffect)(()=>{fetch(`/user`).then(res=>res.json()).then(data=>{a(data.map(u=>({id:u.id,nombre:u.nombre,rol:u.rol===`ADMINISTRADOR`?`Administrador`:u.rol===`MANTENIMIENTO`?`T\u00e9cnico`:`Personal`,correo:u.email,averiasComunicadas:[]})))})},[])';
if (content.includes(ie_anchor)) {
    content = content.replace(ie_anchor, ie_replacement);
} else {
    // Fallback if signature already changed by previous run but rest not
    content = content.replace('function ie({navegar:e,rol_u:rol_u}){let[n,r]=(0,b.useState)(``),[i,a]=(0,b.useState)(w)', ie_replacement);
}
content = content.replace('rolUsuario!==`admin`','!(rol_u==`admin`||rol_u==`ADMINISTRADOR`)');

// 6. ViewPersonal (ae) fetch
